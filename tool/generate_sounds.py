...
def clean_edges(samples, *, fade_in=0.005, fade_out=0.080):
    ...
    The fade-out is a generous 80ms: bells like word/success still carry ~10%
    of their amplitude when the buffer ends, and a short ramp let that tail get
    cut off abruptly. 80ms resolves it to true silence smoothly. Short clips are
    unaffected — [fo] is capped at half the buffer (tap stays ~30ms)."""
    out = list(samples)
    n = len(out)
